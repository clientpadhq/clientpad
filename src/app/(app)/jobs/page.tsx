  ...
  return (
    <div className="space-y-4">
      ...
      {jobs.length === 0 ? <div className="rounded-lg border border-dashed border-slate-300 bg-white p-6 text-sm text-slate-600">No jobs yet.</div> : (
        <ul className="space-y-2">{jobs.map((job:any)=><li key={job.id} className="rounded-lg border border-slate-200 bg-white p-4"><div className="flex items-center justify-between"><Link href={`/jobs/${job.id}`} className="font-medium">{job.title}</Link><StatusBadge status={job.status} /></div><p className="text-sm text-slate-600">{job.client?.business_name ?? 'No client'} • Priority: {job.priority}</p></li>)}</ul>
      )}
    </div>
  );
}
