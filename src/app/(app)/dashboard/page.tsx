  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-4">
        <Card title="Quotes / Invoices"><p className="text-sm">Quotes: {revenue.totalQuotes} (sent {revenue.quotesSent})</p><p className="text-sm">Active invoices: {revenue.activeInvoices}</p></Card>
        <Card title="Outstanding"><p className="text-xl font-semibold">{formatNaira(revenue.outstandingBalance)}</p><p className="text-xs text-slate-500">Overdue invoices: {revenue.overdueInvoices}</p></Card>
        <Card title="Jobs"><p className="text-sm">Active: {execution.activeJobs}</p><p className="text-sm">Due today: {execution.jobsDueToday}</p><p className="text-sm">Overdue: {execution.overdueJobs}</p></Card>
        <Card title="Tasks"><p className="text-sm">Open: {execution.openTasks}</p><p className="text-sm">Due today: {execution.tasksDueToday}</p><p className="text-sm">Overdue: {execution.overdueTasks}</p></Card>
      </div>

      <div className="grid gap-3 md:grid-cols-2">
        <Card title="Accountability"><p className="text-sm">Assigned to me: {execution.assignedToMe}</p><p className="text-sm">Unassigned items: {execution.unassignedItems}</p><p className="text-sm">Reminders due: {execution.remindersDue}</p></Card>
        <Card title="Create Custom Reminder"><form action={createReminderAction} className="space-y-2"><input name="title" placeholder="Reminder title" required /><textarea name="description" placeholder="Details" rows={2} /><input type="datetime-local" name="due_at" /><input name="related_entity_type" placeholder="related entity type (optional)" /><input name="related_entity_id" placeholder="related entity id (optional)" /><button className="w-full bg-slate-800 text-white">Create reminder</button></form></Card>
      </div>
