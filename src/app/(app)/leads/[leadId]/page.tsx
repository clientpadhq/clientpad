...
export default async function LeadDetailPage({ params }: { params: Promise<{ leadId: string }> }) {
  ...
  const latestFollowUp = aiRows.find((row) => row.generation_type === "follow_up_draft" && row.status === "success")?.output_text;

  return (
    <div className="space-y-4">
      <PageHeader title={lead.name} description="Lead details and history" action={<Link className="rounded-md border border-slate-300 px-4 py-2 text-sm" href={`/leads/${lead.id}/edit`}>Edit lead</Link>} />
      ...
      <Card title="Lead Tasks">
        ...
        <ul className="mt-3 space-y-2 text-sm">{(tasks ?? []).map((task: Task) => <li key={task.id} className="rounded border border-slate-200 p-2"><Link className="font-medium" href={`/tasks/${task.id}`}>{task.title}</Link><p className="text-slate-600">{task.status} • {task.priority}</p></li>)}</ul>
      </Card>
      <Card title="Lead Reminders"><ReminderList reminders={reminders ?? []} /></Card>
      <Card title="AI Copilot (Optional)">
        <div className="grid gap-3 md:grid-cols-2">
          <AIGenerateCard title="Lead summarization" description="Paste messy notes and generate a structured summary. Review before applying." generationType="lead_summary" entityType="lead" entityId={leadId} returnPath={`/leads/${leadId}`} context={{ lead_name: lead.name, service_interest: lead.service_interest, notes: lead.notes, raw_notes: "Paste raw chat/call notes here" }} />
          <AIGenerateCard title="Follow-up draft" description="Generate a concise WhatsApp-ready follow-up draft." generationType="follow_up_draft" entityType="lead" entityId={leadId} returnPath={`/leads/${leadId}`} context={{ lead_name: lead.name, status: lead.status, service_interest: lead.service_interest, next_follow_up_at: lead.next_follow_up_at, notes: lead.notes }} />
          <AIGenerateCard title="Next-step suggestions" description="Recommendation-only next actions for this lead." generationType="next_step_suggestion" entityType="lead" entityId={leadId} returnPath={`/leads/${leadId}`} context={{ lead_name: lead.name, status: lead.status, urgency: lead.urgency, budget_clue: lead.budget_clue, next_follow_up_at: lead.next_follow_up_at }} />
        </div>
        <div className="mt-3"><AIHistoryList rows={aiRows} /></div>
      </Card>
      <Card title="WhatsApp Follow-up Share">
        <WhatsAppShareCard title="Share follow-up draft" phone={lead.phone} message={latestFollowUp ?? `Hello ${lead.name}, just following up regarding ${lead.service_interest ?? "your request"}. Kindly share a convenient time to proceed.`} fallbackLabel="Copy follow-up" logPath={JSON.stringify({ workspace_id: workspace.id, entity_type: "lead", entity_id: leadId, activity_type: "follow_up.shared", description: "Lead follow-up shared via WhatsApp" })} />
      </Card>
