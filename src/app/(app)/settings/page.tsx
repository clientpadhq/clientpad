      ...
      <Card title="Workspace Profile">
        {canManageSettings(context.role) ? (
          <form action={updateWorkspaceAction} className="space-y-3">
            ...
            <div className="rounded border border-slate-200 bg-slate-50 p-3 text-sm text-slate-600">Default currency: <span className="font-medium">NGN</span></div>
        ...
        {canManageSettings(context.role) ? (
          <div className="space-y-4">
            <form action={inviteMemberAction} className="grid gap-2 md:grid-cols-3">
              ...
              <select name="role" defaultValue="staff"><option value="owner">owner</option><option value="admin">admin</option><option value="staff">staff</option></select>
                ...
                {members.map((member) => (
                  <li key={member.user_id} className="rounded border border-slate-200 p-3 text-sm">
                    <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                      <div><p className="font-medium text-slate-900">{member.profiles?.full_name ?? member.user_id}</p><p className="text-xs text-slate-500">{member.user_id}</p></div>
                      <form action={updateMemberRoleAction} className="flex items-center gap-2">
                        ...
                        <select name="role" defaultValue={member.role}><option value="owner">owner</option><option value="admin">admin</option><option value="staff">staff</option></select>
            ...
            <div>
              ...
              {invites.length === 0 ? <p className="text-sm text-slate-600">No invites yet.</p> : (
                <ul className="space-y-2 text-sm">
                  {invites.map((invite: any) => (
                    <li key={invite.id} className="rounded border border-slate-200 p-3">
                      <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                        <div><p className="font-medium">{invite.email}</p><p className="text-xs text-slate-500">{invite.role} • {invite.status}</p></div>
                        {invite.status === "pending" ? (
                          <form action={revokeInviteAction}><input type="hidden" name="invite_id" value={invite.id} /><button className="border border-slate-300">Revoke</button></form>
                        ) : null}
                      </div>
                    </li>
                  ))}
