...
type DealClient = { business_name: string | null; phone: string | null };
type DealWithClient = Awaited<ReturnType<typeof getDeal>> & { client?: DealClient | null };

export default async function DealDetailPage({ params }: { params: Promise<{ dealId: string }> }) {
  const { workspace } = await requireWorkspace();
  const { dealId } = await params;
  const deal = (await getDeal(workspace.id, dealId)) as DealWithClient;
  ...
  return (
    <div className="space-y-4">
      <PageHeader title={deal.title} description="Deal details and stage tracking" action={<Link className="rounded-md border border-slate-300 px-4 py-2 text-sm" href={`/deals/${deal.id}/edit`}>Edit deal</Link>} />
      ...
      <Card title="Deal Tasks"><QuickTaskForm entityType="deal" entityId={dealId} titlePrefix={deal.title} /><ul className="mt-3 space-y-2 text-sm">{(tasks ?? []).map((task: Task) => <li key={task.id} className="rounded border border-slate-200 p-2"><Link href={`/tasks/${task.id}`} className="font-medium">{task.title}</Link><div className="mt-1"><StatusBadge status={task.status} /></div></li>)}</ul></Card>
      <Card title="Deal Reminders"><ReminderList reminders={reminders ?? []} /></Card>
      <Card title="AI Copilot (Optional)">
        <div className="grid gap-3 md:grid-cols-2">
          <AIGenerateCard title="Follow-up draft" description="Generate a practical follow-up draft for this deal." generationType="follow_up_draft" entityType="deal" entityId={dealId} returnPath={`/deals/${dealId}`} context={{ deal_title: deal.title, stage: deal.stage?.name, amount: deal.amount, expected_close_date: deal.expected_close_date, notes: deal.notes }} />
          <AIGenerateCard title="Next-step suggestions" description="Get recommendation-only next steps for closing or progressing this deal." generationType="next_step_suggestion" entityType="deal" entityId={dealId} returnPath={`/deals/${dealId}`} context={{ deal_title: deal.title, stage: deal.stage?.name, client: deal.client?.business_name, amount: deal.amount, notes: deal.notes }} />
        </div>
      ...
      <Card title="WhatsApp Follow-up Share">
        <WhatsAppShareCard
          ...
          message={latestFollowUp ?? `Hello ${deal.client?.business_name ?? ""}, following up on deal ${deal.title}. Current stage: ${deal.stage?.name ?? ""}. Let us know next step to move forward.`}
