      ...
      <main className="mx-auto flex min-h-screen w-full max-w-lg items-center px-4 py-10">
        <div className="w-full rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <form action={createWorkspaceAction} className="mt-6 space-y-3">
            ...
            />
            <select className="w-full rounded border border-slate-300 p-2" name="preset_id" defaultValue="">
  ...
  return (
    <main className="mx-auto w-full max-w-2xl px-4 py-10">
      <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <p className="mt-1 text-sm text-slate-600">Complete your setup now, or skip optional steps and resume later.</p>

        {params.error ? <p className="mt-4 rounded bg-red-50 p-2 text-sm text-red-700">{params.error}</p> : null}

        <ol className="mt-6 space-y-2 text-sm">
          <li className={onboardingState.business_profile_completed ? "text-emerald-700" : "text-slate-700"}>1. Business profile basics</li>
          <li className={onboardingState.branding_payment_completed ? "text-emerald-700" : "text-slate-700"}>2. Branding and payment defaults</li>
          <li className={onboardingState.preset_selected ? "text-emerald-700" : "text-slate-700"}>3. Preset selection</li>
          <li className={onboardingState.data_import_completed ? "text-emerald-700" : "text-slate-700"}>4. Optional data import</li>
        ...
        {currentStep === "branding_payment" ? (
          <form action={saveOnboardingStepAction} className="mt-6 space-y-3">
            ...
            <input className="w-full rounded border border-slate-300 p-2" name="phone" defaultValue={workspaceData.workspace.phone ?? ""} placeholder="Business phone" />
            <input className="w-full rounded border border-slate-300 p-2" name="email" placeholder="Business email (optional)" />
            <textarea className="w-full rounded border border-slate-300 p-2" name="address" placeholder="Business address (optional)" rows={2} />
            <textarea className="w-full rounded border border-slate-300 p-2" name="default_invoice_terms" placeholder="Default invoice terms or payment instructions" rows={3} />
            <select className="w-full rounded border border-slate-300 p-2" name="default_currency" defaultValue={workspaceData.workspace.default_currency || "NGN"}>
        ...
        {currentStep === "preset_selection" ? (
          <form action={saveOnboardingStepAction} className="mt-6 space-y-3">
            ...
            <select className="w-full rounded border border-slate-300 p-2" name="selected_preset" defaultValue={onboardingState.selected_preset ?? ""}>
        ...
        {currentStep === "data_import" ? (
          <div className="mt-6 space-y-3">
            <p className="text-sm text-slate-600">Import leads/clients now, or skip and continue.</p>
