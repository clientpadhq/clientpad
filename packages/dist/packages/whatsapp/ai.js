export const BOOKING_KEYWORDS = ["book", "appointment", "tomorrow", "today"];
export const QUOTE_KEYWORDS = ["price", "how much", "cost"];
export const PAYMENT_KEYWORDS = ["pay", "transfer", "paid"];
export function detectDeterministicIntent(message) {
...
export function applyReplySafetyRules(reply, confidenceThreshold = 0.7) {
    const sensitiveCategory = detectSensitiveAdviceCategory(reply.body);
    const requiresOwnerApproval = reply.requiresOwnerApproval || reply.confidence < confidenceThreshold || sensitiveCategory !== null;
    return {
        ...reply,
...
export function detectSensitiveAdviceCategory(text) {
...
function createDeterministicReply(intent) {
    const bodyByIntent = {
