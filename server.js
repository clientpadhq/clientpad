...
function normalizePathKeepExt(url) {
    const raw = url.split("?")[0].split("#")[0];
    let path = raw.replace(/\/+$/, "") || "/";
    if (path.endsWith(".md"))
        return { path: path.slice(0, -3) || "/", ext: ".md" };
    if (path.endsWith(".html"))
        return { path: path.slice(0, -5) || "/", ext: ".html" };
...
function mediaTypeMatches(mediaType, type, subtype) {
...
const server = createServer(async (req, res) => {
    ...
    if (url === "/llms-full.txt" || url === "/llms-full.txt?") {
        if (!llmsFullTxtCache)
            llmsFullTxtCache = await buildLlmsFullTxt();
        res.writeHead(200, {
            "content-type": "text/markdown; charset=utf-8",
            "cache-control": "public, max-age=3600",
            "x-robots-tag": "noindex",
